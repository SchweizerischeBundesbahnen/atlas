...
@Slf4j
@IntegrationTest
class SectorCreateIntegrationTest {
  ...
  void shouldImportTrafficPointsAndCreateViaApi() throws IOException {
    // Given
    when(sectorBulkImportClient.bulkImportCreate(any())).thenAnswer(i -> {
    ...
    File file = ImportFiles.getFileByPath("import-files/valid/sector-create.csv");

    // When
    MockMultipartFile multipartFile = new MockMultipartFile("file", "sector-create", CSV_CONTENT_TYPE,
    ...
    // Then
    verify(sectorBulkImportClient, times(1)).bulkImportCreate(any());
