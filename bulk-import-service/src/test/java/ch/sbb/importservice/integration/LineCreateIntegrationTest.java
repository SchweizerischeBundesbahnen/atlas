...
@Slf4j
@IntegrationTest
class LineCreateIntegrationTest {
  ...
  void shouldImportTrafficPointsAndCreateViaApi() throws IOException {
    // Given
    when(lineBulkImportClient.lineCreate(any())).thenAnswer(i -> {
      List<BulkImportUpdateContainer<LineCreateCsvModel>> argument = i.getArgument(0, List.class);
      return argument.stream().map(j -> BulkImportItemExecutionResult.builder().lineNumber(j.getLineNumber()).build()).toList();
    });
    File file = ImportFiles.getFileByPath("import-files/valid/line-create.csv");

    // When
    MockMultipartFile multipartFile = new MockMultipartFile("file", "line-create", CSV_CONTENT_TYPE,
    ...
    // Then
    verify(lineBulkImportClient, times(1)).lineCreate(any());
