...
public class DossierTuCsvModel {
  ...
  public static DossierTuCsvModel fromDossierAndStatement(Dossier dossier,
      TimetableHearingAnonymStatementCsvModel statement) {
    Optional<DossierQuestion> question = dossier.getDossierQuestions().stream().findFirst();
    String deadlineToAnswer =
        dossier.getBoDeadlineToAnswer() != null ? DATE_FORMATTER_BASE.format(dossier.getBoDeadlineToAnswer()) : null;
    return DossierTuCsvModel.builder()
        .dossierId(dossier.getId())
        .dossierStatus(dossier.getDossierStatus())
        .dossierTopic(dossier.getTopic())
        .deadlineToAnswer(deadlineToAnswer)
        .questionForTU(question.map(DossierQuestion::getQuestion).orElse(null))
        .answerFromTU(question.map(DossierQuestion::getAnswerToCanton).orElse(null))
