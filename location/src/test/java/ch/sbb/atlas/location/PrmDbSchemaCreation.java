...
@Sql(scripts = {
    "/stop-point-schema.sql"}, executionPhase = ExecutionPhase.BEFORE_TEST_METHOD, config = @SqlConfig(dataSource =
    "prmDataSource", transactionManager = "prmTransactionManager", transactionMode = SqlConfig.TransactionMode.ISOLATED))
@Sql(scripts = {
    "/stop-point-drop.sql"}, executionPhase = ExecutionPhase.AFTER_TEST_METHOD, config = @SqlConfig(dataSource = "prmDataSource"
