...
describe('LiDi: Scenario Line-CRUD: New Line', { testIsolation: false }, () => {
  ...
  it('Step-4: Read the line version', () => {
    ...
    CommonUtils.get(`/line-directory/v2/lines/versions/${slnid}`).then(
      (response) => {
        const lineVersionsFirst = ReleaseApiUtils.makeCommonChecks(
          response,
          slnid,
          lineVersionId
        );
        ...
        expect(lineVersionsFirst)
          .to.have.property('swissLineNumber')
          .that.is.a('string');
  ...
  it('Step-5: Update the line version', () => {
    ...
    CommonUtils.put(`/line-directory/v2/lines/versions/${lineVersionId}`, {
      ...
      businessOrganisation: sboid,
      validFrom: ReleaseApiUtils.todayAsAtlasString(),
      validTo: ReleaseApiUtils.atlasDay(56), // Changed, see above
