...
describe(
  'TTH: Create a statement with several email-addresses',
  { testIsolation: false },
  () => {
    const allStatusChoices = [
      TthUtils.status.PLANNED,
      TthUtils.status.ACTIVE,
      TthUtils.status.ARCHIVED,
    ].join(',');
    ...
    it('Step-3: Close active year, if available', () => {
      CommonUtils.get(
        '/line-directory/v1/timetable-hearing/years?statusChoices=' +
          TthUtils.status.ACTIVE
      ).then((response) => {
        ...
        if (response.body.length > 0) {
          ...
          expect(response.body[0])
            .property('timetableYear')
            .to.be.greaterThan(currentYear);

          // Then: ... close the active year
          CommonUtils.post(
            '/line-directory/v1/timetable-hearing/years/' +
              response.body[0].timetableYear +
              '/close',
            {}
          ).then((response) => {
    ...
    it('Step-4: Determine the next free timetable hearing year', () => {
      ...
      CommonUtils.get(
        ...
        const activeYears = tthYears
          .filter((year) => year.hearingStatus === TthUtils.status.ACTIVE)
          .map((year) => year.timetableYear);
        expect(activeYears.length, "There shouldn't be any active year.").to.eq(
          0
        );

        const usedYears = tthYears
          .filter(
            (year) =>
              year.hearingStatus === TthUtils.status.PLANNED ||
              year.hearingStatus === TthUtils.status.ARCHIVED
          )
          .map((year) => year.timetableYear);
    ...
    it('Step-6: Create a timetable hearing year', () => {
      ...
      CommonUtils.post('/line-directory/v1/timetable-hearing/years', {
        ...
        expect(response)
          ...
          .to.eq(TthUtils.status.PLANNED);
      });
    });

    it('Step-7: Start the timetable hearing year', () => {
      CommonUtils.post(
        `/line-directory/v1/timetable-hearing/years/${timetableYear}/start`
      ).then((response) => {
        expect(response).property('status').to.equal(200);
        expect(response)
          .property('body')
          .property('hearingStatus')
          .to.eq(TthUtils.status.ACTIVE);
      });
    });

    it('Step-8: Create statement with 10 email-addresses in active timetable hearing year', () => {
      TthUtils.postStatement(
        '/line-directory/v2/timetable-hearing/statements',
        {
          ttfnid: ttfnId,
          statementSender: {
            emails: [
    ...
    it('Step-9: Get statement values', () => {
      ...
      &ttfnid=${ttfnId}`
        ...
        expect(objects)
          .property('statementSender')
          .to.exist.and.to.haveOwnProperty('emails')
    ...
    it('Step-11: Close the timetable hearing year', () => {
      CommonUtils.post(
        `/line-directory/v1/timetable-hearing/years/${timetableYear}/close`
      ).then((response) => {
        expect(response).property('status').to.equal(200);
        expect(response)
          .property('body')
          .property('hearingStatus')
          .to.eq(TthUtils.status.ARCHIVED);
        expect(response)
          .property('body')
          .property('timetableYear')
          .to.eq(timetableYear);
    ...
    it('Step-12: Check statement is in ARCHIVED timetable year', () => {
      CommonUtils.get(
        `/line-directory/v2/timetable-hearing/statements/${statementId}`
      ).then((response) => {
        expect(response).property('status').to.equal(200);
        expect(response)
          .property('body')
