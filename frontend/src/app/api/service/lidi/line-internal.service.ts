...
export class LineInternalService {
  ...
  public getLines(
    ...
    typeRestrictions?: Array<LidiElementType>,
    elementRestrictions?: Array<ElementType>,
    businessOrganisation?: string,
    validOn?: Date,
    fromDate?: Date,
    toDate?: Date,
    validToFromDate?: Date,
    page?: number,
    size?: number,
    sort?: Array<string>
  ...
  public revokeLine(slnid: string): Observable<void> {
    this.atlasApiService.validateParams({ slnid });
    return this.atlasApiService.post(`${this.INTERNAL_LINES}/${encodeURIComponent(String(slnid))}/revoke`);
  }
  ...
  public checkAffectedSublines(
    id: number,
    updateLineVersionV2: UpdateLineVersionV2
  ): Observable<AffectedSublinesModel> {
