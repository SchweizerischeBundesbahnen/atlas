...
export class SublineInternalService {
  ...
  public revokeSubline(slnid: string): Observable<void> {
    ...
    return this.atlasApiService.post(`${this.INTERNAL_SUBLINES}/${encodeURIComponent(String(slnid))}/revoke`);
  }

  public deleteSublines(slnid: string): Observable<void> {
    this.atlasApiService.validateParams({ slnid });
    return this.atlasApiService.delete(`${this.INTERNAL_SUBLINES}/${encodeURIComponent(String(slnid))}`);
  }
}
