...
import { LineVersionWorkflow, Workflow, WorkflowProcessingStatus, WorkflowStart } from '../../../api';
...
export class LineWorkflowDialogComponent implements OnInit {
  ...
  private initWorkflowReadMode(workflowId: number) {
    ...
    this.lineWorkflowService
      ...
      .subscribe((workflow: Workflow) => this.populateWorkflowStartFormGroupFromPersistence(workflow));
  }
  ...
  private populateWorkflowStartFormGroupFromPersistence(workflow: Workflow) {
    this.workflowStatusTranslated$ = this.translateService.get('WORKFLOW.STATUS.' + workflow.workflowStatus);
