...
export class TableContentPaginationAndSorting {
  static pageAndSort<TYPE>(content: Array<TYPE>, settings: TablePagination, defaultSort: string): Array<TYPE> {
  ...
  static sort<TYPE>(content: Array<TYPE>, settings: TablePagination, defaultSort: string) {
    content.sort((a: any, b: any) => {
      ...
      const direction = (settings.sort?.substring(settings.sort?.indexOf(',') + 1) || 'asc') as 'asc' | 'desc';
