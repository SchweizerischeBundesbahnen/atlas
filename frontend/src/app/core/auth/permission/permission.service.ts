...
export class PermissionService {
  ...
  hasPermissionsToCreate(applicationType: ApplicationType): boolean {
    return PermissionService.hasPermissionsToCreateWithPermissions(applicationType, this.permissions, this.isAdmin);
  }
  ...
  hasWritePermissionsToForCanton(applicationType: ApplicationType, canton: string | undefined): boolean {
    return PermissionService.hasPermissionToWriteOnCanton(applicationType, canton, this.permissions, this.isAdmin);
  }
  ...
  mayAccessTtfn() {
    ...
    return this.isAdmin || [ApplicationRole.Supervisor].includes(applicationUserPermission.role);
  }

  mayAccessBulkImport() {
    return this.isAdmin || BulkImportPermission.hasAnyBulkImportPermission(this.permissions);
  }
  ...
  static hasPermissionsToCreateWithPermissions(
    ...
    const applicationPermission = PermissionService.getApplicationPermission(permissions, applicationType);
  ...
  static hasPermissionToWriteOnCanton(
    ...
    const applicationUserPermission = PermissionService.getApplicationPermission(permissions, applicationType);
  ...
  private static getRolesAllowedToCreate(applicationType: ApplicationType) {
    let rolesAllowedToCreate = [ApplicationRole.Supervisor, ApplicationRole.SuperUser, ApplicationRole.Writer];
  ...
  private static getApplicationPermission(permissions: Permission[], applicationType: ApplicationType): Permission {
    const applicationPermissions = permissions.filter((permission) => permission.application === applicationType);
  ...
  getTerminationPermission(): TerminationDecisionPersonEnum | undefined {
    const applicationUserPermission = this.getApplicationUserPermission(ApplicationType.Sepodi);
    const votingRestriction = applicationUserPermission.permissionRestrictions.find(
      (i) =>
        [PermissionRestrictionType.InfoPlusTerminationVote, PermissionRestrictionType.NovaTerminationVote].includes(
          i.type!
  ...
  isTthBoUser(): boolean {
    const applicationUserPermission = this.getApplicationUserPermission(ApplicationType.TimetableHearing);
    return applicationUserPermission.permissionRestrictions.some(
      (i) => [PermissionRestrictionType.TransportCompanyDossierAnswer].includes(i.type!) && i.valueAsString === 'true'
    );
  }
