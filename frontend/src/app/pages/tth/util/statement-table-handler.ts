...
export abstract class StatementTableHandler {
  ...
  computeDataProtectionIcon(dataProtectionChecked: boolean): string {
    const iconClass = dataProtectionChecked ? 'bi-check color-success' : 'bi-x color-error';
    return `bi ${iconClass}`;
  }

  isDocumentExisting(documents: Array<TimetableHearingStatementDocument>): boolean {
