...
export class SectorFormGroupBuilder {
  static buildFormGroup(sectorVersion?: ReadSectorVersion): FormGroup<SectorDetailFormGroup> {
    return new FormGroup<SectorDetailFormGroup>(
      {
        ...
        designation: new FormControl(sectorVersion?.designation, [Validators.required, Validators.maxLength(8)]),
        length: new FormControl(sectorVersion?.length, [
          AtlasCharsetsValidator.decimalWithDigits(6, 3),
          Validators.min(0),
        ]),
        edgeHeight: new FormControl(sectorVersion?.edgeHeight, [AtlasCharsetsValidator.numeric, Validators.max(999)]),
        sectorGeolocation: GeographyFormGroupBuilder.buildFormGroup(sectorVersion?.sectorGeolocation),
        validFrom: new FormControl(sectorVersion?.validFrom ? moment(sectorVersion.validFrom) : null, [
          Validators.required,
        ]),
        validTo: new FormControl(sectorVersion?.validTo ? moment(sectorVersion.validTo) : null, [Validators.required]),
