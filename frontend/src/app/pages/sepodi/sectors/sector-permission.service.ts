...
import { ApplicationType, MeanOfTransport, ReadServicePointVersion } from '../../../api';
...
export class SectorPermissionService {
  ...
  showCreateButton(servicePoint: ReadServicePointVersion[]) {
    const hasVersionWithMotTrain = servicePoint.some((i) => i.meansOfTransport?.includes(MeanOfTransport.Train));

    const hasPermissionsForOneVersion = servicePoint.some((i) => {
      return (
        this.permissionService.hasPermissionsToWrite(ApplicationType.Sepodi, i.businessOrganisation) &&
