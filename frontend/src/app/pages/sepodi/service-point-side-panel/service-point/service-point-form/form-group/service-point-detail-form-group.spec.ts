...
describe('ServicePointFormGroup', () => {
  ...
  beforeEach(() => {
    servicePointFormGroup = ServicePointFormGroupBuilder.buildEmptyFormGroup(EMPTY);
    ...
    servicePointFormGroup.controls.validityGroup.controls.validFrom.setValue(moment(new Date(2000 - 1 - 1)));
    servicePointFormGroup.controls.validityGroup.controls.validTo.setValue(moment(new Date(2099 - 10 - 1)));
  });
  ...
  it(
    ...
    () => {
      ...
      servicePointFormGroup.controls.spTypeGroup?.controls.stopPoint.setValue(true);
  ...
  it('should add validators to include one of stopPoint, freightServicePoint. freightServicePoint needs nothing', () => {
    ...
    servicePointFormGroup.controls.spTypeGroup?.controls.freightServicePoint.setValue(true);
    expect(servicePointFormGroup.valid).toBe(true);
  });
  ...
  it('should require operatingPointType for OperatingPoint', () => {
    ...
    if (!('operatingPointType' in servicePointFormGroup.controls.spTypeGroup!.controls)) {
      throw 'wrong form group';
    }
    servicePointFormGroup.controls.spTypeGroup?.controls.operatingPointType.setValue(OperatingPointType.InventoryPoint);
    expect(servicePointFormGroup.valid).toBe(true);
  });
  ...
  it('should set RouteNetwork and KilometerMaster undefined when ServicePoint', () => {
    ...
    const createServicePointVersion: CreateServicePointVersion =
      ServicePointFormGroupBuilder.mapper.getWritableServicePoint(servicePointFormGroup);

    expect(createServicePointVersion.operatingPointKilometerMasterNumber).toEqual(undefined);
    expect(createServicePointVersion.operatingPointRouteNetwork).toEqual(undefined);
  });
  ...
  it('should set RouteNetwork true and KilometerMaster undefined when OperatingPoint', () => {
    servicePointFormGroup.controls.selectedType.markAsDirty();
    servicePointFormGroup.controls.selectedType.setValue(ServicePointType.OperatingPoint);
    servicePointFormGroup.controls.routeNetworkGroup?.controls.operatingPointKilometerMaster.setValue(7000);
    servicePointFormGroup.controls.routeNetworkGroup?.controls.operatingPointRouteNetwork.setValue(true);
    const createServicePointVersion: CreateServicePointVersion =
      ServicePointFormGroupBuilder.mapper.getWritableServicePoint(servicePointFormGroup);
  ...
  it('should validate MoT and stopPointType required on existing StopPoint', () => {
    servicePointFormGroup = ServicePointFormGroupBuilder.buildFormGroup(BERN_WYLEREGG, EMPTY);
    if (!('stopPoint' in servicePointFormGroup.controls.spTypeGroup!.controls)) {
      throw 'wrong form group';
    }
    servicePointFormGroup.enable();
    servicePointFormGroup.controls.spTypeGroup?.controls.stopPointGroup?.controls.meansOfTransport.setValue([]);
    servicePointFormGroup.controls.spTypeGroup?.controls.stopPointGroup?.controls.stopPointType.setValue(undefined);
