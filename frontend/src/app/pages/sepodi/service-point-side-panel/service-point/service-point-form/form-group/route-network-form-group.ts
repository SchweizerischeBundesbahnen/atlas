...
import { CreateServicePointVersion, ReadServicePointVersion } from '../../../../../../api';
...
export class RouteNetworkFormGroup {
  static routeNetworkGroup(destroy$: Observable<void>, version?: ReadServicePointVersion) {
    const routeNetworkGroup = new FormGroup<RouteNetworkGroup>({
      operatingPointRouteNetwork: new FormControl(version?.operatingPointRouteNetwork ?? false, { nonNullable: true }),
    ...
    routeNetworkGroup.controls.operatingPointRouteNetwork.valueChanges
      .pipe(
        takeUntil(destroy$),
        filter(() => routeNetworkGroup.controls.operatingPointRouteNetwork.dirty)
      )
      .subscribe((checked) =>
        checked
          ? this.handleOperatingPointRouteNetworkChecked(routeNetworkGroup, version)
          : this.handleOperatingPointRouteNetworkUnchecked(routeNetworkGroup)
      );

    routeNetworkGroup.controls.operatingPointKilometer.valueChanges.pipe(takeUntil(destroy$)).subscribe((value) => {
      if (!value) {
  ...
  private static handleOperatingPointRouteNetworkChecked(
    ...
    routeNetworkGroup.controls.operatingPointKilometerMaster.setValue(version?.number.number, { emitEvent: false });
    routeNetworkGroup.controls.operatingPointKilometerMaster.disable({
      emitEvent: false,
    });
  }

  private static handleOperatingPointRouteNetworkUnchecked(routeNetworkGroup: FormGroup<RouteNetworkGroup>) {
