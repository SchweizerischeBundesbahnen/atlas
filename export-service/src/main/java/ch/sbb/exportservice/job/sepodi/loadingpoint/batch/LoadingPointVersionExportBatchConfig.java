package ch.sbb.exportservice.job.sepodi.loadingpoint.batch;
...
import ch.sbb.exportservice.job.sepodi.loadingpoint.processor.LoadingPointVersionCsvProcessor;
import ch.sbb.exportservice.job.sepodi.loadingpoint.processor.LoadingPointVersionJsonProcessor;
...
import ch.sbb.exportservice.job.sepodi.loadingpoint.writer.CsvLoadingPointVersionWriter;
import ch.sbb.exportservice.job.sepodi.loadingpoint.writer.JsonLoadingPointVersionWriter;
...
public class LoadingPointVersionExportBatchConfig {
  ...
  public JdbcCursorItemReader<LoadingPointVersion> loadingPointReader(
    ...
    JdbcCursorItemReader<LoadingPointVersion> itemReader = new JdbcCursorItemReader<>(dataSource,
        LoadingPointVersionSqlQueryUtil.getSqlQuery(exportTypeV2), new LoadingPointVersionRowMapper());
  ...
  public Step exportLoadingPointCsvStep(ItemReader<LoadingPointVersion> itemReader) {
    ...
    return new StepBuilder(stepName, jobRepository)
        .<LoadingPointVersion, LoadingPointVersionCsvModel>chunk(StepUtil.CHUNK_SIZE).transactionManager(transactionManager)
        .reader(itemReader)
  ...
  public Step exportLoadingPointJsonStep(ItemReader<LoadingPointVersion> itemReader) {
    ...
    return new StepBuilder(stepName, jobRepository)
        .<LoadingPointVersion, ReadLoadingPointVersionModel>chunk(StepUtil.CHUNK_SIZE).transactionManager(transactionManager)
        .reader(itemReader)
