package ch.sbb.exportservice.job.prm.parkinglot.batch;
...
import ch.sbb.exportservice.job.prm.parkinglot.processor.ParkingLotVersionCsvProcessor;
import ch.sbb.exportservice.job.prm.parkinglot.processor.ParkingLotVersionJsonProcessor;
...
import ch.sbb.exportservice.job.prm.parkinglot.writer.CsvParkingLotVersionWriter;
import ch.sbb.exportservice.job.prm.parkinglot.writer.JsonParkingLotVersionWriter;
...
public class ParkingLotVersionExportBatchConfig {
  ...
  public JdbcCursorItemReader<ParkingLotVersion> parkingLotReader(
    ...
    JdbcCursorItemReader<ParkingLotVersion> itemReader = new JdbcCursorItemReader<>(dataSource,
        ParkingLotVersionSqlQueryUtil.getSqlQuery(exportTypeV2), new ParkingLotVersionRowMapper());
  ...
  public Step exportParkingLotCsvStep(ItemReader<ParkingLotVersion> itemReader) {
    ...
    return new StepBuilder(stepName, jobRepository)
        .<ParkingLotVersion, ParkingLotVersionCsvModel>chunk(StepUtil.CHUNK_SIZE).transactionManager(transactionManager)
        .reader(itemReader)
  ...
  @Bean
  @StepScope
  public FileDeletingTaskletV2 deleteParkingLotCsvFileTaskletV2(
  ...
  public Step exportParkingLotJsonStep(ItemReader<ParkingLotVersion> itemReader) {
    ...
    return new StepBuilder(stepName, jobRepository)
        .<ParkingLotVersion, ReadParkingLotVersionModel>chunk(StepUtil.CHUNK_SIZE).transactionManager(transactionManager)
        .reader(itemReader)
  ...
  @Bean
  @StepScope
  public FileDeletingTaskletV2 deleteParkingLotJsonTaskletV2(
      @Value("#{jobExecutionContext[filePathV2]}") ExportFilePathV2 filePathV2) {
    return new FileDeletingTaskletV2(filePathV2);
  }
  // END: Delete Json V2
