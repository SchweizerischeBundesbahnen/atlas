...
class ExportReferencePointIntegrationTest extends BasePrmSqlIntegrationTest {
  ...
  void shouldExecuteExportReferencePointCsvJob() throws Exception {
    ...
    // when
    JobExecution jobExecution = jobOperator.start(exportReferencePointCsvJob, jobParameters);
  ...
  void shouldExecuteExportReferencePointJsonJob() throws Exception {
    ...
    JobParameters jobParameters = BaseExportJobService.buildJobParameters(new JobParams(ExportTypeV2.FULL));
    // when
    JobExecution jobExecution = jobOperator.start(exportReferencePointJsonJob, jobParameters);
    JobInstance actualJobInstance = jobExecution.getJobInstance();
    ExitStatus actualJobExitStatus = jobExecution.getExitStatus();
