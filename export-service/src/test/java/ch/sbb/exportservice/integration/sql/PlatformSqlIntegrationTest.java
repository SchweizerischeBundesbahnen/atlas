...
class PlatformSqlIntegrationTest extends BasePrmSqlIntegrationTest {
  ...
  void shouldReturnTimetableYears() throws SQLException {
    ...
    insertPlatform(2, "ch:1:sloid:7001:1", ServicePointNumber.ofNumberWithoutCheckDigit(8507000), actualTimetableYearChangeDate,
        actualTimetableYearChangeDate);
    insertPlatform(20, "ch:1:sloid:7001:1", ServicePointNumber.ofNumberWithoutCheckDigit(8507000),
        actualTimetableYearChangeDate.minusMonths(5),
        actualTimetableYearChangeDate.minusMonths(4));
    insertPlatform(200, "ch:1:sloid:7001:1", ServicePointNumber.ofNumberWithoutCheckDigit(8507000),
        actualTimetableYearChangeDate.plusMonths(4),
        actualTimetableYearChangeDate.plusMonths(5));
    insertPlatform(2000, "ch:1:sloid:7001:1", ServicePointNumber.ofNumberWithoutCheckDigit(8507000), LocalDate.of(1999, 1, 1),
        LocalDate.of(2010, 1, 1));

    final String sqlQuery = PlatformVersionSqlQueryUtil.getSqlQuery(ExportTypeV2.TIMETABLE_YEARS);

    // when
    final List<PlatformVersion> result = executeQuery(sqlQuery);
